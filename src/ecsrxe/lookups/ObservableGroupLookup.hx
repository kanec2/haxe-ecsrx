...
/** 
 * Lookup for observable groups keyed by their token. 
 * Mimics the behavior of KeyedCollection<ObservableGroupToken, IObservableGroup> from C#. 
 */
class ObservableGroupLookup {
	// Карта для быстрого поиска по ключу (token)
	final map:Map<ObservableGroupToken, IObservableGroup>;
	// Массив для хранения элементов в порядке добавления и итерации
	final list:Array<IObservableGroup>;

	public function new() {
		map = new Map<ObservableGroupToken, IObservableGroup>();
		list = [];
	}
	 ...
	 */
	public function add(item:IObservableGroup):Void {
		if (item == null) {
	 ...
	 */
	public function remove(key:ObservableGroupToken):Bool {
		if (map.exists(key)) {
	 ...
	 */
	public function get(key:ObservableGroupToken):IObservableGroup {
		if (map.exists(key)) {
			return map.get(key);
		}
	 ...
	 */
	public function contains(key:ObservableGroupToken):Bool {
	 ...
	 */
	public function getByIndex(index:Int):IObservableGroup {
	 ...
	 */
	function getKeyForItem(item:IObservableGroup):ObservableGroupToken {
		// В C# это был абстрактный метод protected abstract TKey GetKeyForItem(TItem item);
		// В Haxe мы делаем его виртуальным и реализуем логику по умолчанию
		if (item == null) {
			throw "Item cannot be null";
		}
		// Предполагаем, что IObservableGroup имеет свойство token
		return item.token;
	}

	/** 
	 * Returns an iterator over the items in the lookup. 
	 * This makes the lookup Iterable<IObservableGroup>. 
	 */
	public function iterator():Iterator<IObservableGroup> {
	 ...
	 */
	public function toArray():Array<IObservableGroup> {
		return list.copy();
	}
}